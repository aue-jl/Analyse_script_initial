...
knit: (function(inputFile, encoding) { 
  rmarkdown::render(inputFile,
                    encoding=encoding, 
                    output_file='Rapport_Etude_Maille_Habitat.html') })
---
...
- **POP** : population
- **LOG** : nombre de logements (Source DV3F)
- **tot_lgmt** : nombre de logements (Source FILOCOM)
...
<br>
<span style="color:red"><strong>Note : </strong> l'étude du CGDD comporte une erreur dans les données AirDNA : le taux de locations touristiques apparaît à 0 % au lieu de 17,5 % sur la commune de Calcatoggio. Le présent script utilise la même valeur erronée (0 %) afin de retrouver les mêmes résultats que le CGDD. Cette valeur à été modifiée directement dans le fichier Excel source : "Données/stat_airdna_aout2020_mod.xlsx" </span>
...
```{r traceSecteurs_diff, echo=T, eval=F ,fig.align='center',fig.show='hold' ,fig.width = 9, fig.height = 9, eval=F}
#nb_mailles=36
#population_min=2500
...
## Modifications du script n°2 : sectorisation sans maillage préalable
...
Ainsi, on choisit d'ajouter à notre table de données la population par commune.
...
### Suppression des mailles : essai 3


Dans cet essai on réalise le regroupement hiérarchique sur 5 secteurs.
...
- Sectorisation sans maillage préalable
- Ajout de la population dans les données d'entrée
- Ajout d'un 5 ème secteur


```{r sectorisationSansMaillage_Pop_5secteurs,echo=F, eval=T, results=F, include = F}
rm(list=ls())
...
hc5_communes <- HCPC(ACP_parCommunes, nb.clust = 5, consol = T, graph = F)
cl5_communes <- data.frame(CODGEO = Indicateurs_parCommune$CODGEO, cl_maille5 = hc5_communes$data.clust[,"clust"])


carteCommunesSecteurs <- left_join(carteCommunesCorses, cl5_communes, by = "CODGEO")
...
# Ajout des mailles avec un trait noir plus épais et colorées selon le secteur
carte8b=carteSecteursCommunes %>%
  addPolygons(data = carteCommunesSecteurs_simplified,
              fillColor = ~palSecteurs(cl_maille5),
              fillOpacity = 0.8,
              color = "black",
              weight = 2)
widget_file8b <- "Cartes_Interactives/carte_interactive_8b.html"
saveWidget(carte8b, file = widget_file8b)
webshot("Cartes_Interactives/carte_interactive_8b.html", file = "Cartes_Interactives/carte8b.png",vwidth = 860, vheight = 860)
...
<br>
*Cliquer sur l'image pour afficher la carte interactive*
<a href="Cartes_Interactives/carte_interactive_8b.html" target="_blank"><img src="Cartes_Interactives/carte8b.png" alt=" "></a>

<br>

## Modifications du script n°3
