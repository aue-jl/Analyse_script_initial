...
# Mise à jour et ajout de nouveaux indicateurs Airdna

## Essai n°1

Synthhèse des modifications (par rapport au script initial du CGDD) :

- Mise à jour de **indic.airbnb** avec les données de 2022
- Ajout d'un indicateur **indic.airbnb_RevenuTotal** correspondant au chiffre d'affaires total sur 2022 des locations touristiques de courte durée 





```{r codeMofifie_MAJ_Airdna_1,echo=F, eval=T, include = F}
rm(list=ls())
...
ImportsAirDNA2022=read.csv("Données/Airdna0223_pf_logmt_actLTM_grpby_com.csv",sep=";")
...
airDNA=ImportsAirDNA2022 %>% select(1,3,8)
colnames(airDNA)=c("CODGEO","Loc.actives","Revenu_Tot")
Donnees_Logement_2017=Donnees_Logement_2017 %>% left_join(airDNA, by="CODGEO")
...
Indicateurs_parCommune= Donnees_Logement_2017 %>% 
  mutate(indic.transac = round(100 * nb_mut / LOG, 1),
         ...
         indic.prix = round(100 * moyenne_prix/rev_ucm, 1),
         indic.airbnb=round(100*Loc.actives/LOG, 1),
         indic.airbnb_RevenuTotal=Revenu_Tot) %>% 
  select(CODGEO,LIBGEO,POP,LOG,contains("indic"))
Indicateurs_parCommune=Indicateurs_parCommune %>% mutate_if(is.numeric, funs(ifelse(is.na(.), 0, .)))
...
widget_file9 <- "Cartes_Interactives/carte_interactive_10.html"
saveWidget(carte10, file = widget_file9)
webshot("Cartes_Interactives/carte_interactive_10.html", file = "Cartes_Interactives/carte10.png",vwidth = 860, vheight = 860)
